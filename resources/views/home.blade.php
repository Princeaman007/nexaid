...
@section('content')
    <!-- Section Hero -->
    <div style="background-color: #f8f9fa; padding: 40px 20px; border-radius: 8px; text-align: center; margin-bottom: 40px;">
        <h1 style="font-size: 2.5em; color: #343a40; margin-bottom: 20px;">
            @if(app()->getLocale() == 'fr')
        ...
        <p style="font-size: 1.2em; color: #6c757d; max-width: 800px; margin: 0 auto 30px auto;">
            @if(app()->getLocale() == 'fr')
                Nous connectons les étudiants talentueux avec des opportunités de stage exceptionnelles à travers le monde. Commencez votre aventure internationale dès aujourd'hui.
            @else
                We connect talented students with exceptional internship opportunities around the world. Start your international adventure today.
            @endif
        </p>
    ...
    <div style="margin-bottom: 40px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px;">
            @if(isset($internships) && count($internships))
                    ...
                    <div style="border: 1px solid #dee2e6; border-radius: 8px; overflow: hidden;">
                        @if($internship->image)
                            <div style="height: 150px; overflow: hidden;">
                                <img src="{{ asset('storage/' . $internship->image) }}" alt="{{ $internship->title[app()->getLocale()] ?? '' }}" style="width: 100%; height: 100%; object-fit: cover;">
                            </div>
                        @endif
                        <div style="padding: 20px;">
                            ...
                            <div style="display: flex; flex-wrap: wrap; margin-bottom: 10px;">
                                @if($internship->location)
                                    <span style="background-color: #e9ecef; padding: 5px 10px; border-radius: 20px; font-size: 0.9em; margin-right: 10px; margin-bottom: 5px;">
                                        <i class="fas fa-map-marker-alt"></i> {{ $internship->location }}
                                    </span>
                                @endif
                                
                                @if($internship->category)
                                    <span style="background-color: #e9ecef; padding: 5px 10px; border-radius: 20px; font-size: 0.9em; margin-bottom: 5px;">
                                        {{ $internship->category->name[app()->getLocale()] ?? '' }}
                                    </span>
                                @endif
                            </div>
                            
                            <p style="color: #6c757d; margin-bottom: 15px;">{{ \Illuminate\Support\Str::limit(strip_tags($internship->description[app()->getLocale()] ?? ''), 100) }}</p>
                            
                            <a href="{{ route('internships.show', $internship->slug) }}" style="color: #007bff; text-decoration: none; font-weight: bold;">
                                @if(app()->getLocale() == 'fr')
                                    Voir les détails →
                                @else
                                    View details →
                ...
                @endforeach
            @else
                <div style="grid-column: 1 / -1; text-align: center; padding: 30px; background-color: #f8f9fa; border-radius: 5px;">
    ...
    <div style="margin-bottom: 40px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 20px;">
            <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; text-align: center;">
                ...
                <a href="{{ route('services.internship-search') }}" style="color: #007bff; text-decoration: none; font-weight: bold;">
                    @if(app()->getLocale() == 'fr')
                        En savoir plus →
                    @else
                        Learn more →
            ...
            <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; text-align: center;">
                ...
                <a href="{{ route('services.language-courses') }}" style="color: #007bff; text-decoration: none; font-weight: bold;">
                    @if(app()->getLocale() == 'fr')
                        En savoir plus →
                    @else
                        Learn more →
            ...
            <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; text-align: center;">
                ...
                <a href="{{ route('services.airport-pickup') }}" style="color: #007bff; text-decoration: none; font-weight: bold;">
                    @if(app()->getLocale() == 'fr')
                        En savoir plus →
                    @else
                        Learn more →
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px;">
                @foreach ($blogs as $blog)
                    <div style="border: 1px solid #dee2e6; border-radius: 8px; overflow: hidden;">
                        @if($blog->image)
                            <div style="height: 150px; overflow: hidden;">
                                <img src="{{ asset('storage/' . $blog->image) }}" alt="{{ $blog->title[app()->getLocale()] ?? '' }}" style="width: 100%; height: 100%; object-fit: cover;">
                            </div>
                        @endif
                        <div style="padding: 20px;">
                            ...
                            <p style="color: #6c757d; margin-bottom: 15px;">{{ \Illuminate\Support\Str::limit(strip_tags($blog->content[app()->getLocale()] ?? ''), 100) }}</p>
                            
                            <a href="{{ route('blog.show', $blog->slug) }}" style="color: #007bff; text-decoration: none; font-weight: bold;">
                                @if(app()->getLocale() == 'fr')
                                    Lire l'article →
                                @else
                                    Read article →
                    ...
                    <div style="text-align: center;">
                        @if($partner->logo)
                            <a href="{{ $partner->website_url ?? '#' }}" target="_blank">
                                <img src="{{ asset('storage/' . $partner->logo) }}" alt="{{ $partner->name[app()->getLocale()] ?? '' }}" style="max-height: 80px; max-width: 150px;">
                            </a>
                        @else
    ...
    <div style="background-color: #007bff; color: white; text-align: center; padding: 40px 20px; border-radius: 8px;">
        ...
        <a href="{{ route('internships.index') }}" style="background-color: white; color: #007bff; text-decoration: none; padding: 12px 30px; border-radius: 4px; font-size: 1.1em; display: inline-block; font-weight: bold;">
            @if(app()->getLocale() == 'fr')
